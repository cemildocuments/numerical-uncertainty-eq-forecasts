...
def main():
    ...
    sources=['run_forecasts.py','forecast_context.py','adaptive_threshold_probability.py','threshold_probability.py','controlled_mean_probability.py','validated_probability.py','validate_ball_zero.py','conventional_quadrature.py','branching_monte_carlo.py']
    ...
    with ProcessPoolExecutor(max_workers=execution['workers']) as pool:
        for origin_index in sorted(set(int(row['origin_index']) for row in rows)):
            subset=[row for row in rows if int(row['origin_index'])==origin_index]
            origin=timestamp_ns(subset[0]['origin_utc']);past=past_events(events,origin)
            history=model_history(past,origin,fit['parameters']['m0']);pending={}
            for row in subset:
                meta=dict(row);meta['run_numerical_comparators']=execution.get('run_numerical_comparators',True);meta['past_year_targets']=count_window(past,origin-365*DAY_NS,origin,row['magnitude_threshold'])
                if int(row['numerical_check']):meta['numerical_case_index']=counter;counter+=1
                filename=f"{origin_index:04d}_h{row['horizon_days']}_m{row['magnitude_threshold']}.json";path=out/filename
                if path.exists():
                ...
                pending[pool.submit(worker,(c,meta,execution['policy'],numerical,rates[row['magnitude_threshold']]))]=path
            for future in as_completed(pending):
                value=future.result();value['signature']=signature;atomic(pending[future],value);done+=1
                state['completed_cases']=done;atomic(out/'manifest.json',state)
            print(json.dumps(dict(origin_index=origin_index,completed_cases=done,total_cases=len(rows))),flush=True)
